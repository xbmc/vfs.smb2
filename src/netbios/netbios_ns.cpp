...
struct netbios_ns
{
    ...
    uint16_t            last_trn_id = 0;  // Last transaction id used;
    NS_ENTRY_QUEUE      entry_queue = { 0 };
    uint8_t             buffer[RECV_BUFFER_SIZE] = { 0 };
#ifdef HAVE_PIPE
    int                 abort_pipe[2];
#else
    std::recursive_mutex abort_lock;
    bool                aborted = false;
#endif
    unsigned int        discover_broadcast_timeout = 0;
    std::thread         discover_thread;
    bool                discover_started = false;
...
static bool   netbios_ns_is_aborted(netbios_ns *ns)
{
    std::lock_guard<std::recursive_mutex> locker(ns->abort_lock);
    bool res = ns->aborted;
    return res;
}
...
static ssize_t netbios_ns_send_packet(netbios_ns* ns, netbios_query* q, uint32_t ip)
{
    ...
    return sendto(ns->socket, (char *)q->packet,
                               ...
                               netbios_ns_name_query *out_name_query)
{
    ...
    while (true)
    {
        ...
        else if (FD_ISSET(sock, &read_fds))
        {
            ...
            size = recvfrom(sock, (char*)ns->buffer, RECV_BUFFER_SIZE, 0,
...
static netbios_ns_entry *netbios_ns_entry_add(netbios_ns *ns, uint32_t ip)
{
    ...
    entry = (netbios_ns_entry *)calloc(1, sizeof(netbios_ns_entry));
    if (!entry)
        return NULL;
...
{
    netbios_ns *ns = new(std::nothrow) netbios_ns;
    if (!ns)
        return NULL;
                              ...
                              netbios_ns_discover_callbacks *callbacks)
{
    ...
    ns->discover_thread = std::thread(netbios_ns_discover_thread, ns);
